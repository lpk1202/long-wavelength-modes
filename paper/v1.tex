...
We begin with a brief review of standard perturbation theory and its detailed formalism up to second-order in section \ref{sec1}. Then we build the quadratic estimator and compute signal-to-noise of the estimator in section \ref{sec2}. In \ref{sec3} we present a concise study of the estimator with data from N-body simulation. Finally we conclude this work and discuss future work related to this in section \ref{sec4}. We use $\Lambda$CDM universe with the Planck Collaboration XVI (2014) \cite{Planck:2014cos} cosmological parameters in this work (same as parameters of the N-body simulation).
...
Compute the correlation function of two short-wavelength modes $\vk_s$ and $\vk_s'$, in the squeezed limit $\vk_l=\vk_s+\vk_s'$ up to second-order ($\tau$ is fixed):
\bea 
...
{\delta^{(1)}}(\vk_{s}) {\delta^{(1)}}(\vk_{s}'){\delta^{(1)}}(\vk_l') 
\rangle
=\langle {\delta^{(1)}}(\vk_{s}) {\delta^{(1)}}(\vk_{s}') \rangle {\delta^{(1)}}(\vk_l') 
\ee 
...
\Sfig{SN}{Detectability of long-wavelength modes. We use a toy survey of boxsize $8.85\, \rm Gpc$.}
