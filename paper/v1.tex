...
We begin with a brief review of standard perturbation theory and its detailed formalism up to second-order in section \ref{sec1}. Then we build the quadratic estimator and compute signal-to-noise of the estimator in section \ref{sec2}. In \ref{sec3} we present a concise study of the estimator with data from N-body simulation. Finally we discuss applications of the estimator and future work related to this in section \ref{sec4}. We use $\Lambda$CDM universe with the Planck Collaboration XVI (2014) \cite{Planck:2014cos} cosmological parameters in this work (same as parameters of the N-body simulation).

\section{Standard Perturbation Theory}\label{sec1}
In this section we review the SPT approach of describing small scale nonlinearity. In the case of a perfect pressurelesee fluid, the nonrelativistic cosmological fluid equations are continuity, Euler and Poisson equations:
\bea
...
\eea 
This approach gives a nearly accurate description of $\Lambda$CDM universe \cite{Takahashi:2008to} with a slight generalization:
\bea
...
{\delta^{(1)}}(\vk_s) {\delta^{(1)}}(\vk_s'){\delta^{(1)}}(\vk_l) 
\rangle
=\langle {\delta^{(1)}}(\vk_s) {\delta^{(1)}}(\vk_s') \rangle {\delta^{(1)}}(\vk_l) 
\ee 
Since in a real life survey, we can only measure this long-wavelength mode limited times. Thus it won't have any statistical property and we can safely extract the long-wavelength mode out of the bracket.\\
...
\Sfig{SN}{Detectability of long-wavelength modes. We use a toy survey of boxsize $6\, h\rm Mpc^{-1}$.}

\section{N-Body Simulation}\label{sec3}
We test the power of the quadratic estimator using data from snapshot at $z=0$ of BigMPDL, one of the MultiDark cosmological simulations \cite{Klypin:2014nov}. Boxsize of BigMDPL is $2.5\, h\rm Mpc^{-1}$, where $h=0.6777$. \\
...
\acknowledgements
\noindent We thank Adam Solomon, Andresa Campos, Duncan Campbell, Fabian Schmidt and Lam Hui for resourceful discussions.  SD, PL and RC were supported by U.S.\ Dept.\ of Energy contract DE-SC0019248.\\
